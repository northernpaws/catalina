...
use super::{ConstHz, FromIterator, Phase, Signal};
use crate::audio::frame::Frame;
use crate::audio::sample::Sample;
use crate::audio::window::Window as WindowType;
    ...
    W: WindowType<f64, Output = f64>,
{
    ...
    pub fn new(len: usize) -> Self {
        let step = super::rate(len as f64 - 1.0).const_hz(1.0);
        Window {
            phase: super::phase(step),
            marker: PhantomData,
        }
    }
}
    ...
    W: WindowType<f64, Output = f64>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.bin <= num_frames {
            ...
            Some(Windowed {
                signal: super::from_iter(frames.iter().cloned()),
                window: window,
            })
        } else {
            None
...
mod tests {
    use crate::audio::frame::Frame;
    use crate::audio::signal::window::{self, Windower};
