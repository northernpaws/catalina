...
//!
//! The slice types where adapted from [dasp](https://github.com/RustAudio/dasp/tree/master)
...
pub use boxed::{
    ...
    FromBoxedSampleSlice, ToBoxedFrameSlice, ToBoxedSampleSlice, from_boxed_frame_slice,
    from_boxed_sample_slice, to_boxed_frame_slice, to_boxed_sample_slice,
};

pub use frame::{
    DuplexFrameSlice, DuplexFrameSliceMut, FromFrameSlice, FromFrameSliceMut, ToFrameSlice,
    ToFrameSliceMut, from_frame_slice, from_frame_slice_mut, to_frame_slice, to_frame_slice_mut,
};
...
mod tests {
    ...
    fn test_add_slice_with_amp_per_channel() {
        ...
        let b = [[1.0]; 32];
        let amp = [0.5];
        super::add_in_place_with_amp_per_channel(&mut a, &b, amp);
        assert_eq!([[1.0]; 32], a);
    }
